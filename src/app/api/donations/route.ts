...
async function sendCertificateEmail(donation: { fullName: string; email: string; amount: number; treeCount: number; NGO: { name: string }; createdAt: Date }) {
  ...
  // 1. SendGrid ile e-posta gönderme (öncelikli)
  if (process.env.SENDGRID_API_KEY) {
    try {
      ...
      const msg = {
        to: donation.email,
        from: 'noreply@yesilgelecek.com', // Doğrulanmış domain kullanın
  ...
  // 2. Resend ile e-posta gönderme (yedek)
  if (process.env.RESEND_API_KEY) {
    try {
      ...
      if (error) {
        ...
        console.log('✅ Resend e-posta başarıyla gönderildi:', data);
      }
      return;
    } catch (error) {
  ...
  // 3. Fallback: Test ortamı için Ethereal Email
  console.log('📧 E-posta servisi bulunamadı, test e-postası gönderiliyor...');
